...
contract MyContract {
    ...
    modifier isOwner() {
        require(
            msg.sender == Owner,
            "Unauthorized. Only contract's owner can perform this operation."
        );
        _;
    }
    ...
    function CeoFunding(uint256 _amount) public payable isOwner {
        require(msg.value >= _amount, "Input is not enough.");
    }
    ...
    function getDetailBetOfPlayer(uint256 index)
        public
        view
        returns (Bet memory)
    {
    ...
    function CeoWithdraw(uint256 _amount) external {
        ...
        require(
            _amount <= address(this).balance - rewardAmount,
            "Can not withdraw an amount of money that exceeds frozen fund."
        );
        msg.sender.transfer(_amount);
    }

    function betting(uint8 _choice, uint256 _amount) external payable {
        require(msg.value >= _amount, "Input not enough.");
        require(
            (address(this).balance - rewardAmount) >=
                (_amount * 3 - _amount / 10),
            "Contract does not have enough money to ensure reward of your bet."
        );
        ...
        require(
            ...
            "User's choice must be in range."
        );

        // push user request to BET
        users[msg.sender].push(
            Bet(_amount, now, ref.latestAnswer(), _choice, 0, -1)
        );
    ...
    function claim(uint256 _index) public {
        require(
            now >= users[msg.sender][_index].timeStart + 3600,
            "Must claim at least 1 hour after betting."
        );
        require(
            users[msg.sender][_index].status == 0,
            "Bet had been done already."
        );
        ...
        if (
            (ref.latestTimestamp() <=
                users[msg.sender][_index].timeStart + 3600)
        ) {
            // win
            if (
                ((users[msg.sender][_index].lastPrice < ref.latestAnswer()) &&
                    users[msg.sender][_index].choice == 0) ||
                ((users[msg.sender][_index].lastPrice == ref.latestAnswer()) &&
                    users[msg.sender][_index].choice == 1) ||
                ((users[msg.sender][_index].lastPrice > ref.latestAnswer()) &&
                    users[msg.sender][_index].choice == 2)
            ) {
                users[msg.sender][_index].isWin = 1;
                // transfer reward
                msg.sender.transfer(
                    users[msg.sender][_index].amount *
                        3 -
                        users[msg.sender][_index].amount /
                        10
                );
            ...
            rewardAmount -= (users[msg.sender][_index].amount *
                3 -
                users[msg.sender][_index].amount /
                10);
        } else {
            ...
            for (uint256 i = 1; i <= 60; i++) {
                if (
                    (ref.getTimestamp(latest - i) <=
                        users[msg.sender][_index].timeStart + 3600) &&
                    (users[msg.sender][_index].timeStart + 3600) <
                    (ref.getTimestamp(latest - i + 1))
                ) {
                    // win
                    if (
                        ((users[msg.sender][_index].lastPrice <
                            ref.getAnswer(latest - i)) &&
                            users[msg.sender][_index].choice == 0) ||
                        ((users[msg.sender][_index].lastPrice ==
                            ref.getAnswer(latest - i)) &&
                            users[msg.sender][_index].choice == 1) ||
